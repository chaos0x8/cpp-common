...
TEST_F(AlgorithmTestSuite_all, returnsTrueWhenAllOfElementsFullfilsCondition)
{
  EXPECT_THAT(all(data, ge(1)), Eq(true));
  EXPECT_THAT(all(raw_data, ge(1)), Eq(true));
}

TEST_F(AlgorithmTestSuite_all, returnsFalseWhenAnyOfElementsDoesntFullfilCondition)
{
...
TEST_F(AlgorithmTestSuite_transform, returnsDifferentTypesWithSpecifiesContainer_withRawData)
{
  const std::list<std::string> res = transform<std::list>(raw_data, functor());
  EXPECT_THAT(res, ElementsAre("1", "3", "4"));
}
...
TEST_F(AlgorithmTestSuite_filter, returnsDifferentTypesWithSpecifiesContainer_withRawData)
{
...
TEST_F(AlgorithmTestSuite_first, returnsFirstElementWhichFullfilsCondition_withRawData)
{
  auto res = first(raw_data, ge(2));

  ASSERT_THAT(res, Not(Eq(std::end(raw_data))));
  ASSERT_THAT(*res, Eq(3));
}

TEST_F(AlgorithmTestSuite_first, returnsEndIteratorWhenElementIsNotFound_withRawData)
{
  auto res = first(raw_data, ge(5));

  ASSERT_THAT(res, Eq(std::end(raw_data)));
}
...
TEST_F(AlgorithmTestSuite_max, throwsWhenInputContainerIsEmpty)
{
  ASSERT_THROW(max(std::vector<int>{}), Common::Exceptions::NoResult);
}
...
TEST_F(AlgorithmTestSuite_erase, removesElementFromContainerUsingFunctor)
{
...
TEST_F(AlgorithmTestSuite_forEach, executesStructMethod)
{
  ...
  ASSERT_THAT(fooData, ElementsAre(Field(&Foo::x, Eq(42)), Field(&Foo::x, Eq(42)), Field(&Foo::x, Eq(42))));
}

TEST_F(AlgorithmTestSuite_forEach, executesStructMethodFromPointer)
{
  Common::Algorithm::forEach(fooPtrData, &Foo::bar);
  ASSERT_THAT(fooPtrData, ElementsAre(Field(&Foo::x, Eq(42)), Field(&Foo::x, Eq(42)), Field(&Foo::x, Eq(42))));
}
...
TEST_F(AlgorithmTestSuite_forEach, executesLambda)
{
  ...
  ASSERT_THAT(fooData, ElementsAre(Field(&Foo::y, Eq(13)), Field(&Foo::y, Eq(13)), Field(&Foo::y, Eq(13))));
}
